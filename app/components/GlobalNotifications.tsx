  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-2xl border px-4 py-3 shadow-2xl backdrop-blur-xl w-80
        ${isSuccess
          ? 'border-[#c8ff00]/30 bg-gray-950/95'
          : 'border-white/10 bg-gray-950/90'}`}
      style={{ boxShadow: isSuccess ? '0 8px 32px rgba(0,0,0,0.5), 0 0 0 1px rgba(200,255,0,0.1)' : '0 8px 32px rgba(0,0,0,0.5)' }}
    >
      {/* Icon */}
      <span className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg
        ${isSuccess ? 'bg-[#c8ff00]/15 text-[#c8ff00]' : 'bg-cyan-500/15 text-cyan-400'}`}>
        {isSuccess ? (
...
const TYPE_STYLE: Record<string, { bg: string; text: string; icon: React.ReactNode }> = {
  success: { bg: 'bg-[#c8ff00]/10', text: 'text-[#c8ff00]', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M20 6L9 17l-5-5"/></svg> },
  warning: { bg: 'bg-yellow-500/10', text: 'text-yellow-400', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> },
  error:   { bg: 'bg-red-500/10',    text: 'text-red-400',    icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg> },
  info:    { bg: 'bg-cyan-500/10',   text: 'text-cyan-400',   icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> },
};
...
export default function GlobalNotifications() {
  ...
  const refreshNotifications = useCallback(async () => {
    ...
    // Find truly new notifications to toast
  ...
  // Init: check login, then start polling
  useEffect(() => {
    fetch('/api/auth/me')
  ...
  return (
    <>
      {/* Toast stack — bottom-left to avoid overlapping bell */}
      {toasts.length > 0 && (
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        {panelOpen && (
          <>
            ...
            <div
              className="w-80 rounded-2xl border border-white/10 bg-gray-950/95 backdrop-blur-xl shadow-2xl overflow-hidden"
            ...
            >
              ...
              <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between">
                ...
                {unread > 0 && (
                  <button onClick={markAllRead} className="text-[11px] text-gray-500 hover:text-white transition-colors">
                    อ่านทั้งหมด
                  </button>
                )}
              </div>
                ...
                {notifications.length === 0 ? (
                  ...
                  <ul className="divide-y divide-white/[0.04]">
                    {notifications.map(n => {
                      ...
                      return (
                        <li key={n.notification_id} className={`flex items-start gap-3 px-4 py-3 ${!n.is_read ? 'bg-white/[0.03]' : ''}`}>
